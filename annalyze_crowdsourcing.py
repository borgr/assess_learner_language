...
# all:[lambda results: np.mean(results, axis = 1).flatten(), lambda
# results: np.mean(results > COVERAGE_GOAL, axis = 1).flatten()]
COVERAGE_METHODS = [lambda results: np.mean(results, axis=1).flatten()]
MEAN_MEASURE = "Mean coverage"
# all:["Mean coverage", "Probabillity of more than " + str(COVERAGE_GOAL)
# + " coverage "]
MEASURE_NAMES = [MEAN_MEASURE]
# x
...
def main():
    ...
    coverage_by_corrections_num = assess_coverage(
        False, show=show_coverage, save=save_coverage, res_type=EXACT_COMP)
    if TASK == GEC:
...
def read_simplification():
    ...
    for root, dirs, files in os.walk(TURKERS_DIR):
        for filename in files:
            cur_db = pd.read_table(
                TURKERS_DIR + filename, names=["index", ORIGIN, 1, 2, 3, 4, 5, 6, 7, 8])
            db.append(cur_db)
    db = pd.concat(db, ignore_index=True)
    db.drop("index", inplace=True, axis=1)
    db.dropna(inplace=True, axis=0)
    db.applymap(normalize_sentence)
    return db
...
def convert_correction_to_m2(source, correct, annotator_num=0):
    ...
    while i < len(s) or j < len(c):
        ...
        if error:
            ...
            lines.append("A " + str(span[0]) + " " + str(span[1]) + "|||" + error +
...
def assess_coverage(only_different_samples, show=True, save=True, res_type=EXACT_COMP, res_measure=MEAN_MEASURE, accuracyax=None):
    ...
    if show or save:
        ...
        for comparison_method_key, dist in enumerate(all_ys):
            axes = [plt.subplot("1" + str(axis_num) + str(i + 1))
                    for i in range(axis_num)]
            if save:
                fig_prefix = COMPARISON_METHODS[
                    comparison_method_key] + "_" + repeat
                save = PLOTS_DIR + fig_prefix + r"_coverage" + ".png"
            title_addition = "using " + \
                COMPARISON_METHODS[comparison_method_key] + " comparison"
            plot_coverage_for_each_sentence(
                dist, axes, title_addition, show, save, xlabel)
            ...
            plot_expected_best_coverage(dist, plt.subplot(
                "111"), title_addition, show, save, xlabel)
            plt.cla()
            ...
            plot_covered_corrections_distribution([correction for correction in CORRECTION_NUMS if correction > 0], dist, plt.subplot(
                "111"), title_addition, show, save, xlabel)
        if save:
            fig_prefix = repeat[1:]
...
def ranges_for_poisson_binomial_probability_mass(ps, mass, belief=1, approximate=False):
    ...
    while right_ind <= n and right_covered < right_mass:
        right_covered += get_probability_with_belief(
            ps, right_ind, belief, approximate)
        right_ind += 1
    right_ind -= 1
    while left_ind >= 0 and left_covered < left_mass:
        left_covered += get_probability_with_belief(
            ps, left_ind, belief, approximate)
        left_ind -= 1
    left_ind += 1
...
def compute_coverage(cdf, p, distribution, samples, only_different_samples):
    ...
    while sampled < samples and not cmath.isclose(covered, 1.0):
        ...
        covered += min(uncovered_variants_num[chosen],
                       samples - sampled) * distribution[PROB_COL][chosen]
...
def compute_probability_to_account_async(distribution, samples, repetitions, only_different_samples):
    ...
    # it = pool.imap(lambda x: compute_coverage(cdf, probs, distribution,
    # samples, only_different_samples), list(range(repetitions)))
    it = []
    it += list(pool.imap(__compute_coverage,
                         [(cdf, probs, distribution, samples, only_different_samples)] * repetitions))
    pool.close()
    pool.join()
    return np.array(it)
...
def compute_probability_to_account(distribution, samples, repetitions, only_different_samples):
    ...
    for i in range(repetitions):
        covered[i] = compute_coverage(
            cdf, probs, distribution, samples, only_different_samples)
    return covered
...
def compare_correction_distributions(db, name, index=CORRECTED_SENTENCES_COL, show=True, save=True):
    ...
    if show or save:
        ...
        if save:
            plt.savefig(PLOTS_DIR + fig_prefix + r"_hist" +
        ...
        if save:
            plt.savefig(PLOTS_DIR + fig_prefix +
                        r"percentage_hist" + ".png", bbox_inches='tight')
        if show:
            plt.show()
...
def export_hists(l, data, comparison_by, HISTS_DIR):
    for i, name in enumerate(l):
        ...
        filename = HISTS_DIR + filename + "_" + \
            comparison_by + "_" + INPUT_HIST_IDENTIFIER + ".txt"
...
def plot_covered_corrections_distribution(corrections_to_plot, dist, ax, title_addition="", show=True, save_name=None, xlabel=None):
    ...
    for correction_index, correction_num in enumerate(CORRECTION_NUMS):
        if correction_num in corrections_to_plot:
            ps = np.fromiter((coverage[correction_index]
                              for coverage in coverage_by_corrections_num), np.float)
...
def plot_significance(show=True, save=True):
    ...
    for i, file in enumerate(files):
        print(file, results[i])
    names = [filename if filename !=
             ACL2016RozovskayaRothOutput_file else "RoRo" for filename in files]
    names = [name if name != char_based_file else "char" for name in names]
    results = [[[1, 0, 0], [1, 0, 0]]] + \
        list(results) + [[[1, 1, 1], [1, 1, 1]]]
    ...
    if show or save:
        ...
        for threshold in thresholds:
            ...
            for dist_key, dist in enumerate(dists):
                ...
                # print(variants_num)
                # raise
            mean_freqs.append(np.mean(variants_num))
            mean_masses.append(np.mean(masses))
            print("mean number of variants with more than",
                  threshold, "frequency:", mean_freqs[-1])
            print("mean mass of those variants", mean_masses[-1])
        # plt.clf()
        # plt.semilogy(thresholds, mean_freqs)
        # plt.semilogy(thresholds, mean_masses)
        # plt.show()
        for dist_key, dist in enumerate(dists):
            dist = dist[:, dist[0, :].argsort()[::-1]]
        ...
        if save:
            plt.savefig(PLOTS_DIR + dists_type + "_dists_plot" +
                        ".png", bbox_inches='tight')
        if show:
            plt.show()
        plt.cla()
...
def plot_coverage_for_each_sentence(dist, axes, title_addition="", show=True, save_name=None, xlabel=None):
    ...
    for sent_key, ys in enumerate(dist):
        ...
        for i, y in enumerate(ys):
            ...
            if xlabel:
                ax.set_xlabel(xlabel)
            # ax.set_title(MEASURE_NAMES[i] + " of different amount of
            # corrections\n" + title_addition)
    if save_name:
        plt.savefig(save_name, bbox_inches='tight')
    if show:
...
def plot_acounts_for_percentage(l, ax, data, comparison_by, bottom=1, reverseXY=False):
    ...
    for i, name in enumerate(l):
        ...
        plt.xlim(xmin=0)
        # ax.bar(x + i*width, y, width=width, color=colors[i], align='center',
        # label=name, edgecolor=colors[i])
    plt.autoscale(enable=True, axis='x', tight=False)
...
def plot_differences_hist(l, ax, data, comparison_by, bottom=1, percentage=False):
    ...
    for i, name in enumerate(l):
        ...
        ax.bar(x + i * width, y, width=width,
               color=colors[i], align='center', label=name[:5], edgecolor=colors[i])
    plt.autoscale(enable=True, axis='x', tight=False)
    if percentage:
...
def clean_data(db, 	max_no_correction_needed=8):
    ...
    db.loc[:, CORRECTED_SENTENCES_COL] = db[
        CORRECTED_SENTENCES_COL].apply(normalize_sentence)
    db.loc[:, LEARNER_SENTENCES_COL] = db[
        LEARNER_SENTENCES_COL].apply(normalize_sentence)
    # ignore sentences that many annotators say no corrections are needed for
    # them
...
def get_trial_num(create_if_needed=True):
    ...
    trial_indicators = (COMPARISON_METHODS, MEASURE_NAMES,
                        REPETITIONS, CORRECTION_NUMS)
    trials = []
