...
def main():
    print(align.align("what has is by the meaning of the word is",
    ...
    pool = Pool(POOL_SIZE)
    if r2s:
        results = pool.starmap(distances, zip(
            goals, sources, names), chunksize)
    else:
        results = pool.starmap(distances, zip(
            sources, goals, names), chunksize)
    print(results)
    pool.close()
    pool.join()
    sym_mat = []
    ...
    send_mail("leshem.choshen@mail.huji.ac.il",
              "finished", os.path.abspath(output_path))
    return


def send_mail(adress, subject, attachment, content="..."):
    command = "echo " + content + " | mail -s " + \
        subject + adress + "-a " + attachment
